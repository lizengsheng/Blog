...
文章支持 Markdown 和 MDX 中的数学公式：行内使用 `$E=mc^2$`，独立公式使用单独两行 `$$` 包围公式。公式通过 KaTeX 在构建时生成，字体随站点发布；普通文字中的美元符号应写为 `\$`。公式语法错误需在发布前修正。

流程图直接写在语言标记为 `mermaid` 的围栏代码块中，例如：
...
图表接近视口时才加载本地 Mermaid 并生成 SVG；切换文章会清理旧任务，切换明暗主题时更新当前附近的图表，其余图表在滚动接近时更新。禁用脚本或渲染失败时保留可读源码。复杂图表和长公式可横向滚动；发布前应在手机宽度检查文字和连线。

长文使用一个页面主标题，正文从 `##` 开始。无需添加 `## 目录`：页面直接使用 Astro 构建时生成的二至四级标题，宽屏显示右侧粘性目录，窄屏显示目录按钮与抽屉；无 JavaScript 时仍可使用原生折叠目录。代码中的注释与井号不作为文章标题处理。

正文图片默认延迟加载和异步解码。代码复制由统一页面脚本处理，按钮偏移由 CSS 决定；新增交互应保持初始化幂等，在 `astro:before-swap` 清理监听器，避免在逐块 DOM 修改之间读取计算样式。性能检查应区分正文就绪与全部远程图片加载完成。长文目录使用即时定位，避免超长平滑滚动触发沿途图片加载后落点偏移。
